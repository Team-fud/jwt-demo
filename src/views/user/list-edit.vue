    ...
    <el-dialog
      ...
      <el-form v-loading="submitLoading" ref="terminal" :model="terminal" :rules="userRules" label-width="100px" label-suffix="：" status-icon>
        ...
        <el-form-item label="名称" prop="cateName">
          <el-input v-model.trim="terminal.cateName" placeholder="请输入终端名称"/>
        </el-form-item>
    ...
    submitForm() {
      ...
      this.$refs.terminal.validate((valid) => {
        if (valid) {
          if (this.terminal.id) {
            // userId不为空更新
            user.update(this.terminal).then(response => {
              // 执行成功重置表单且弹出提示信息
              this.resetForm()
              this.$message({
                type: 'success',
            ...
            user.add(this.terminal).then(response => {
              // 执行成功重置表单且弹出提示信息
              this.resetForm()
              this.$message({
                type: 'success',
